...
void commandLine::saveParameters(){
	...
	myfile<<"numofWeights:\t\t\t"<<numWeight<<endl;
	myfile<<"Rang of a:\t\t\t";
	for(auto i:aRange){
		myfile<<i<<" ";
	}
...
void commandLine::initiateA(){
	...
	uniform_real_distribution<double> dist(-numOfInVecTraining/2,numOfInVecTraining/2);		//a between 0-10 by uniform distribution
	aS=dist(mt);
	//	a=5;
...
void commandLine_test()
{
	...
	cout<<"numofWeight= "<< Cl.numWeight<<endl;
